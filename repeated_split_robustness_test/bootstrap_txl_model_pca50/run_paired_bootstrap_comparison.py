...
def resolve_split_dirs(split_name: str):
    return (
        ...
        EXP_ROOT / 'txl_model_pca50' / split_name / 'output',
        EXP_ROOT / 'bootstrap_txl_model_pca50' / split_name,
    )
...
def metric_delta(y_true, pred_xgb, pred_txl):
    deltas = {}
    class_map = [('tsm', SEMIMETAL_LABEL), ('ti', TI_LABEL), ('trivial', TRIVIAL_LABEL)]
...
def main():
    ...
    for split_name, pred_file in [('validation', 'validation_predictions.json'), ('heldout_test', 'heldout_test_predictions.json')]:
        xgb_path = xgb_dir / pred_file
        txl_path = txl_dir / pred_file
        if not xgb_path.exists():
            raise FileNotFoundError(f'Missing XGB prediction file for {args.split_name}: {xgb_path}')
        if not txl_path.exists():
            raise FileNotFoundError(
                f'Missing TXL PCA50 prediction file for {args.split_name}: {txl_path}. '
                'Run repeated_split_robustness_test/run_txl_model_pca50_all_splits.sh first.'
            )
        y_true_xgb, pred_xgb = load_prediction_arrays(xgb_path)
        y_true_txl, pred_txl = load_prediction_arrays(txl_path)
