...
def train_one(mod, config, X, y, train_idx, val_idx):
    ...
    metrics = mod.evaluate(y[val_idx], pred)
    return {
        'config': config['name'],
        ...
        'val_ti_precision': metrics['topological']['precision'],
        'val_ti_recall': metrics['topological']['recall'],
        'val_ti_f1': metrics['topological']['f1-score'],
    }


def main():
    mod = load_module(XGB_SCRIPT, 'weighted_xgb_for_imbalance_val')
    rows = mod.load_json(mod.OUT / 'xgb_data_weighted_shared_split.json')
    ...
    y = np.asarray([label_to_code[label] for label in y_labels], dtype=int)
    train_idx, val_idx = mod.load_shared_split(mod.load_json(mod.TRAINING_DATA_PATH))
    summaries = [train_one(mod, cfg, X, y, train_idx, val_idx) for cfg in CONFIGS]
    pd.DataFrame(summaries).to_csv(BASE / 'xgb_class_imbalance_summary.csv', index=False)
