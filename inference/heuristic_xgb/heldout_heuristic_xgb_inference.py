...
XGB_MODEL_PATH = ROOT / 'weighted_version' / 'heuristic_xgb' / 'xgb_model.json'
FEATURE_NAMES_PATH = ROOT / 'weighted_version' / 'heuristic_xgb' / 'xgb_feature_names.json'
...
def main():
    mod = load_module(HXGB_SCRIPT, 'weighted_heuristic_xgb_module_for_inference')
    train_data = mod.load_json(mod.TRAINING_DATA_PATH)
    test_data = mod.load_json(TEST_DATA_PATH)
    trivial_topogivities = mod.load_json(mod.CLEAN_TOPOGIVITY_DIR / 'trivial_topogivities.json')
    sm_topogivities = mod.load_json(mod.CLEAN_TOPOGIVITY_DIR / 'sm_topogivities.json')
    ...
    feature_rows = mod.build_feature_rows(
        test_data,
        sg_class_counts,
        sg_total_counts,
